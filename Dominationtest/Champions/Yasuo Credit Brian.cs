...
namespace Yasuo
{
    ...
    internal class Yasuo : Logichelper
    {
        ...
        static void autoQ()
        {
            var getobj = GameObjects.AttackableUnits.Where(i => i.IsValidTarget(900) && !i.IsDead);
            if(getobj != null)
            {
                var getheroes = GameObjects.EnemyHeroes.Where(i => i.IsValidTarget(800) && !i.IsDead);
                foreach(var hero in getheroes)
                {
                    if(hero != null)
                    {
                        if (hero.DistanceToPlayer() < objPlayer.GetRealAutoAttackRange() || hero.DistanceToPlayer() < QCirWidthMin - hero.BoundingRadius - 50)
                        {
                            if (objPlayer.IsDashing() && Q.IsReady())
                            {
                                if (inQ13cc()) Q.Cast(PosExploit(hero));
                            }
                        }else
                        {
                            if (HaveQ3 || !Q.IsReady()) return;

                            if (HaveQ2 && inQ13cc()) Q.Cast(PosAfterE(hero));
                            if (!HaveQ2 && !hero.IsValidTarget(Q.Range - 100) && inQ13cc()) Q.Cast(PosExploit(hero));
                        }
                    } 
                    if(hero == null)
                    {
                        if(objPlayer.IsDashing() && Q.IsReady())
                        {
                            foreach(var obj in getobj.Where(i => i.IsValidTarget(QCirWidthMin - 50)))
                            {
                                if(obj != null)
                                {
                                    Q.Cast(PosExploit((AIBaseClient)obj));
                                }
                            }
                            if (HaveQ2) Q.Cast(new Vector3(50000f, 50000f, 50000f), false);
                        }
                    }
                }
            }
