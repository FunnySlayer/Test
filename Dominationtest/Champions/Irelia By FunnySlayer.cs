...
namespace Template
{
    ...
    public class loaded
    {
        ...
        public static void OnLoad()
        {
            ...
            E.SetSkillshot(0.3f, 50f, 1800f, false, false, SkillshotType.Line);
        ...
        {
            foreach(AIBaseClient Objects in GameObjects.EnemyMinions.Where(i => (i.Health < Q.GetDamage(i) || i.HasBuff("ireliamark"))
            && i.IsValidTarget(600)
            && (i.Distance(target) <= objPlayer.Distance(target) + 150 || i.Distance(target) <= objPlayer.GetRealAutoAttackRange())
            ))
            {
                if(Objects != null && Objects.Health < Q.GetDamage(Objects))
                {
                    Q.Cast(Objects);
            ...
            foreach (AIBaseClient Objects in GameObjects.EnemyHeroes.Where(i => (i.Health < Q.GetDamage(i) || i.HasBuff("ireliamark"))
             && i.IsValidTarget(600)
             && (i.Distance(target) <= objPlayer.Distance(target) + 150 || i.Distance(target) <= objPlayer.GetRealAutoAttackRange())
            ))
            {
            ...
            foreach (AIBaseClient Objects in GameObjects.Jungle.Where(i => (i.Health < Q.GetDamage(i) || i.HasBuff("ireliamark"))
             && i.IsValidTarget(600)
             && (i.Distance(target) <= objPlayer.Distance(target) + 150 || i.Distance(target) <= objPlayer.GetRealAutoAttackRange())
            ))
            {
